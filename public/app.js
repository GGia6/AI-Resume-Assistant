...
$("#job-form").addEventListener("submit", async (event) => { event.preventDefault(); const data = Object.fromEntries(new FormData(event.target)); try { await request("/api/jobs", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) }); event.target.reset(); await render(); } catch (e) { alert(e.message); } });
document.addEventListener("click", async (event) => { const button = event.target.closest("button"); if (!button) return; try { if (button.dataset.job) await request("/api/applications", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ jobId: Number(button.dataset.job) }) }); if (button.dataset.approve) await request(`/api/applications/${button.dataset.approve}/approve`, { method: "POST" }); if (button.dataset.submit) await request(`/api/applications/${button.dataset.submit}/submit`, { method: "POST" }); await render(); } catch (e) { alert(e.message); } });
